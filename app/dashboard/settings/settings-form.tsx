...
export function SettingsForm({ preferences }: Props) {
  const [startingBalance, setStartingBalance] = useState<string>(
    preferences?.current_account_starting_balance.toString() || '0'
  )
  const [startingDate, setStartingDate] = useState<string>(
    preferences?.current_account_starting_date || new Date().toISOString().split('T')[0]
  )
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await updateUserPreferences({
      current_account_starting_balance: parseFloat(startingBalance) || 0,
      current_account_starting_date: startingDate || null,
    })
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            The balance of your checking account before you started tracking transactions
          </p>
        </div>
        ...
        <div>
          ...
          />
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            The date of your starting balance snapshot
          </p>
        </div>
      ...
      <div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
        ...
        <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
          <li>• Your <strong>Current Account balance</strong> = Starting Balance + All transactions (minus transfers)</li>
          <li>• This affects your <strong>Total Net Worth</strong> calculation</li>
          <li>• Update this when you want to adjust your baseline</li>
        </ul>
      </div>
    </div>
  )
